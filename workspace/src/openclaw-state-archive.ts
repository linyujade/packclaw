...
const ARCHIVE_MARKER_NAME = ".packclaw-openclaw-state-archive";
const ARCHIVE_MARKER_CONTENT = "packclaw-openclaw-state-archive/v1\n";
...
export async function exportOpenclawStateToArchive(
  ...
  const snapshotRoot = fs.mkdtempSync(path.join(os.tmpdir(), "packclaw-openclaw-export-"));
...
function validateArchiveMarker(entryNames: string[], entryContents: Map<string, Buffer>): void {
  if (!entryNames.includes(ARCHIVE_MARKER_NAME)) {
    throw new Error("不是 PackClaw .openclaw 数据包");
  }
