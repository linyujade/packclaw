...
# 清理 PackClaw 的所有运行痕迹，用于"第一次启动"测试（含 Setup 向导）。
...
BUNDLE_ID="com.packclaw.app"

# 终止 PackClaw 进程（包括 Electron 主进程和 gateway 子进程）
echo "终止 PackClaw 进程"
run killall PackClaw 2>/dev/null || true
run killall -9 PackClaw 2>/dev/null || true
sleep 0.5
