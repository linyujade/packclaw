...
!macro customInit
  ; 安装前强制终止正在运行的 PackClaw 进程树（/T 杀子进程，/F 强制）
  nsExec::ExecToLog 'taskkill /IM "PackClaw.exe" /T /F'
  ; 补杀残留的 gateway 子进程（PackClaw Helper.exe 是 Electron 复用二进制跑 Node.js 的）
  ; /T 有时无法级联到 windowsHide 模式创建的子进程，需显式按进程名清理
  nsExec::ExecToLog 'taskkill /IM "PackClaw Helper.exe" /F'
  ; 补杀 CLI 进程（更新时可能正在运行）
  nsExec::ExecToLog 'taskkill /IM "PackClaw-CLI.exe" /F'
  ; 等待进程退出和文件句柄释放
  Sleep 2000
!macroend
...
!macro customUnInstallSection
  ...
  Section "un.删除命令行工具 (openclaw CLI)"
    ; 删除当前版本 wrapper（%LOCALAPPDATA%\PackClaw\bin\）
    Delete "$LOCALAPPDATA\PackClaw\bin\openclaw.cmd"
    Delete "$LOCALAPPDATA\PackClaw\bin\clawhub.cmd"
    RMDir "$LOCALAPPDATA\PackClaw\bin"
    RMDir "$LOCALAPPDATA\PackClaw"
    ...
    FileOpen $0 "$TEMP\packclaw-uninstall-path.ps1" w
    ...
    FileWrite $0 "}$\r$\n"
    FileWrite $0 "Remove-FromPath $$env:LOCALAPPDATA\PackClaw\bin$\r$\n"
    FileWrite $0 "Remove-FromPath $$env:USERPROFILE\.openclaw\bin$\r$\n"
    FileClose $0

    nsExec::ExecToLog 'powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$TEMP\packclaw-uninstall-path.ps1"'
    Delete "$TEMP\packclaw-uninstall-path.ps1"
  SectionEnd
